...
import 'screens/imc_calculator_screen.dart';
...
class _CalculosSectionState extends State<CalculosSection> {
  ...
  // Only IMC for first approval (no medical/dosage calculators per Guideline 1.4.1/1.4.2)
  static final List<CalculoItem> _allItems = [
    ...
    // --- Removed for App Store: Creatinine Clearance (medical/lab) ---
    // const CalculoItem(
  ...
  void _openEditMode() {
    showModalBottomSheet(
      ...
      builder: (context) => EditItemsSheet(allItems: _allItems, store: _store),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(child: _buildGrid(visibleItems, isSplitView: false)),
          ],
        ),
      ),
    );
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  _buildHeader(hasAnyResult),
                  const SizedBox(height: 16),
                  Expanded(child: _buildGrid(visibleItems, isSplitView: true)),
                ],
              ),
            ),
          ),
  ...
  Widget _buildGrid(List<CalculoItem> items, {required bool isSplitView}) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final result = item.storeKey != null
            ? _store.getResult(item.storeKey!)
            : null;
        final classification = item.storeKey != null
            ? _store.getClassification(item.storeKey!)
            : null;
        final isSelected =
            isSplitView && _selectedItem?.storeKey == item.storeKey;
