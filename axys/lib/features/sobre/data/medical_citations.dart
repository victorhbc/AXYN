...
class MedicalCitations {
  ...
  static const List<CitationEntry> allCitations = [
    // --- Commented for Apple review: only IMC and Creatinine citations ---
    // // Glasgow Coma Scale
    // CitationEntry(
    ...
    CitationEntry(
      ...
      description:
          'Quetelet Index (BMI) - World Health Organization classification',
  ...
  static List<CitationEntry> getCitationsForCalculator(String calculatorName) {
    switch (calculatorName.toLowerCase()) {
      // --- Commented for Apple review: only IMC and Creatinine ---
      // case 'glasgow':
      ...
      case 'bmi':
        return allCitations
            .where(
              (c) => c.title.contains('BMI') || c.title.contains('Body Mass'),
            )
            .toList();
      case 'clearance':
      case 'creatinine':
